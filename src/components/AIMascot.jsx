...
export default function AIMascot({ size = 150, color = '#00FF88', isThinking = false }) {
    return (
        <motion.div
            animate={{ y: [0, -10, 0] }}
        ...
        >
            <svg viewBox="0 0 100 100" width="100%" height="100%" fill="none">
                {/* Legs/Thrusters */}
                <motion.rect
                    x="20" y="60" width="10" height="20" rx="5" fill="#333"
                    animate={isThinking ? { y: [60, 58, 60] } : {}}
                    transition={{ repeat: Infinity, duration: 0.5 }}
                />
                <motion.rect
                    x="70" y="60" width="10" height="20" rx="5" fill="#333"
                    animate={isThinking ? { y: [60, 58, 60] } : {}}
                ...
                <motion.circle
                    cx="45" cy="28" r="2" fill={color}
                    animate={{
                        cx: [40, 60, 40],
                        opacity: isThinking ? [1, 0.5, 1] : 1
                    }}
                    transition={{
                        cx: { repeat: Infinity, duration: isThinking ? 1 : 2, ease: "linear" },
                ...
                <circle cx="40" cy="55" r="2" fill={color} fillOpacity="0.5" />
                <circle cx="45" cy="55" r="2" fill={color} fillOpacity="0.5" />
            </svg>
        </motion.div>
    );
}
